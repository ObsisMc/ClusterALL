...
def test(model, dataset, split_idx, evaluator):
    ...
    data.y, y_pred = testing_loader.convert(data.y), testing_loader.convert(y_pred)
...
def main():
    ...
    data = Data(x=data.x, edge_index=edge_index)
    dataset = GNNClusterDataset(dataset, data, split_idx, num_parts=args.num_parts)
    training_loader = GNNClusterLoader(dataset, "train", is_eval=False, batch_size=-1, shuffle=False)

    for run in range(args.runs):
        model.reset_parameters(dataset.get_init_vnode(device))
        optimizer = torch.optim.Adam(model.parameters(), lr=args.lr)
        for epoch in range(1, 1 + args.epochs):
            loss = train(model, training_loader, optimizer)  # Modify
