...
class NodeformerCluster(AbstractClusteror):
    def __init__(self, encoder: nn.Module, in_channels, hidden_channels, out_channels, decode_channels, **kwargs):
        super().__init__(encoder, in_channels, hidden_channels, out_channels, decode_channels, **kwargs)
...
class NodeformerClusterLoader(AbstractClusterLoader):
    def __init__(self, dataset: AbstractClusterDataset, split_name: str, is_eval, batch_size=-1, shuffle=False):
