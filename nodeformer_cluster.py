...
class ClusterNodeFormerConv(nn.Module):
    '''
    ...
    def __init__(self, in_channels, out_channels, num_heads, kernel_transformation=softmax_kernel_transformation,
                 ...
                 use_edge_loss=True):
        super(ClusterNodeFormerConv, self).__init__()
...
class ClusterNodeFormer(nn.Module):
    '''
    ...
    def __init__(self, in_channels, hidden_channels, num_layers=2, num_heads=4, dropout=0.0,
                 ...
                 use_edge_loss=True):
        super(ClusterNodeFormer, self).__init__()
        ...
        for i in range(num_layers):
            self.convs.append(
                ClusterNodeFormerConv(hidden_channels, hidden_channels, num_heads=num_heads,
    ...
    def forward(self, x, adjs, tau=1.0):
        ...
        layer_ = []
        link_loss_ = torch.zeros((len(self.convs),)).float().to(x.device)
        z = self.fcs[0](x)
        if self.use_bn:
            z = self.bns[0](z)
        z = self.activation(z)
        ...
        if self.use_jk:  # use jk connection for each layer
            x_out = torch.cat(layer_, dim=-1)
        else:
            x_out = z
        x_out = x_out.squeeze(0)  # (N, H)
